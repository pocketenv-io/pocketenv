...
import { Devbox, RunloopSDK } from "@runloop/api-client";
...
async function checkIfServerInstalled(sandbox: Devbox) {
  const exists = await sandbox.cmd.exec(`command -v ${SERVER_BIN_NAME}`);
...
async function setupSandboxEnvironment(
...
): Promise<{ sandbox: Devbox }> {
  const sdk = new RunloopSDK({
    bearerToken: options.apiKey,
  });
  consola.info("Runloop: fetching sandbox", chalk.greenBright(options.id));
  const sandbox = sdk.devbox.fromId(options.id);
  consola.info("Runloop: sandbox fetched", chalk.greenBright(options.id));

  consola.info(
    "Runloop: checking pty-tunnel-server",
  ...
  if (!(await checkIfServerInstalled(sandbox))) {
    ...
    const pathname = `pty-server-${crypto.randomUUID()}`;
    await sandbox.file.upload({
      path: pathname,
      file: new File(
        [await fs.readFile(`/tmp/${SERVER_BIN_NAME}`)],
        SERVER_BIN_NAME,
      ),
    });
    ...
    await sandbox.cmd.exec(
      `bash -c mv "${pathname}" /usr/bin/${SERVER_BIN_NAME} || sudo mv "${pathname}" /usr/bin/${SERVER_BIN_NAME}; chmod a+x /usr/bin/${SERVER_BIN_NAME} || sudo chmod a+x /usr/bin/${SERVER_BIN_NAME}`,
    );
  ...
  await sandbox.cmd.execAsync(
    `bash -c "TERM==${TERM} ${SERVER_BIN_NAME} --port=${PTY_PORT} --mode=client --cols=${process.stdout.columns ?? 80} --rows=${process.stdout.rows ?? 24} bash"`,
    {
      stdout: stdout,
      stderr: stderr,
    },
  ...
  consola.info(
    "Runloop: pty-tunnel-server process started",
    chalk.greenBright(options.id),
  );
...
export async function createTerminalSession(
  ...
  if (!record?.runloop_auth) {
    consola.error("Runloop auth not found for sandbox", { id });
    throw new Error("Runloop auth not found for sandbox " + id);
  }
  ...
  const { sandbox } = await setupSandboxEnvironment(
    {
      ...
      apiKey: decrypt(record.runloop_auth.apiKey),
    },
    ...
    (data) => consola.debug(`pty-tunnel-server [stderr]:`, data.trimEnd()),
  );

  consola.info("Runloop: fetching sandbox tunnels", chalk.greenBright(id));
  const tunnel = (await sandbox.getInfo()).tunnel;
  console.log(
    `Tunnel URL for port ${PTY_PORT}: https://${PTY_PORT}-${tunnel?.tunnel_key}.tunnel.runloop.ai`,
  );
  consola.info(
    "Runloop: awaiting pty-tunnel connection info",
    chalk.greenBright(id),
  );
  const details = await listener.connection;
  consola.info(
    "Runloop: pty-tunnel connection info received",
    chalk.greenBright(id),
  );

  const url =
    `wss://${PTY_PORT}-${tunnel?.tunnel_key}.tunnel.runloop.ai` as const;
  ...
  consola.info(
    "Runloop: waiting for pty-tunnel socket to open",
    chalk.greenBright(id),
  );
  await socket.waitForOpen();
  consola.info(
    "Runloop: pty-tunnel socket open, sending ready",
    chalk.greenBright(id),
  );
